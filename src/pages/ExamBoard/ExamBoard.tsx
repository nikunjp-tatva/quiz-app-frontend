...
export default function ExamBoard() {
    ...
    let blocker: any = useBlocker(
        ({ currentLocation, nextLocation }) =>
            isExamInProgress && currentLocation.pathname !== nextLocation.pathname,
    );

    const addRequiredFieldsInQuestions = (questionData: QuestionDataType[]) => {
        const updatedQuestionList = questionData.map(
            (question: QuestionDataType, index: number) => {
        ...
        return updatedQuestionList.reduce(
            (prev: any, question: QuestionDataType) => {
    ...
    const handleQuestionChange = ({ target }: any) =>
        setCurrentPageNo((prevPageNo) => {
    ...
    const handleMarkReviewButtonClick = () =>
    ...
    const handleUnMarkReviewButtonClick = () =>
        setQuestionsData((prev: any) => {
            const questionData = prev[currentPageNo];
    ...
    return !isExamStart ? (
        ...
        <Box sx={{ display: "flex", height: "calc(100vh - 80px)" }}>
            {blocker.state === "blocked" ? (
                <ConfirmationDialog
                    open={isExamInProgress}
                    handleConfirm={examProgressSaveConfirmation}
                    handleClose={examProgressNotSave}
                />
            ) : null}
            <Grid container direction="row" sx={{ width: "100%", height: "100%" }}>
                ...
                >
                    ...
                    <QuestionData
                        questionDetails={questionsData[currentPageNo]}
                        onOptionSelect={handleQuestionOptionChange}
                    />
                    ...
                    <Box
                        ...
                        pt={1}
                        pb={1}
                        sx={{ borderTopWidth: 1, backgroundColor: COLOR.NOT_ATTEMPTED }}
                    >
                        <Box
                            pt={1}
                            pb={1}
                        ...
                        >
                            <Button
                                ...
                                onClick={
                                    questionsData[currentPageNo]?.status === "MarkForReview"
                                        ? handleUnMarkReviewButtonClick
                                        : handleMarkReviewButtonClick
                                }
                            >
                                Mark for review
                            </Button>
                            <Box sx={{ display: "flex", justifyContent: "center", gap: 0.5 }}>
                                ...
                                <Button
                                    ...
                                    disabled={
                                        Number(currentPageNo) === Object.keys(questionsData).length
                                    }
                                >
                                    Next
                                </Button>
                ...
                <Grid item md={3.98} lg={3.28} xl={2.98}>
                    ...
                    {technologyDetails?.duration && (
                        <ExamTimer
                            examTime={technologyDetails?.duration}
                            onTimerEnds={handleSubmitButtonClickEvent}
                        />
                    )}
